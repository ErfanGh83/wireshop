...
const AddressModule = ({ setModuleIsOpen }: Props) => {
    ...
    useEffect(() => {
        // Get addresses from backend (for later)
        // Initialize the selected address from local storage
    ...
    useEffect(() => {
        // Save the selected address in the local storage
        if (selectedAddress) {
    ...
    return (
        <div className='w-full max-w-[95vw] sm:w-[500px] md:w-[600px] h-[400px] md:h-[500px] flex flex-col relative rounded-md overflow-hidden bg-white shadow-lg'>
            <button
                    ...
                    addresses.map((address) => (
                        ...
                        >
                            <input
                                ...
                                className="mt-1 accent-blue-500 cursor-pointer" // Added accent-blue-500 for blue radio
                                style={{ width: '18px', height: '18px' }} // Optional: Adjust size
                            />
            ...
            <button className='w-full h-fit p-3 md:p-4 cursor-pointer text-sm md:text-base text-white bg-blue-500 hover:bg-blue-600 transition-colors border-t'>
                افزودن آدرس جدید
            </button>
        </div>
    )
